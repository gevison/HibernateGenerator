...
public class SwimlaneDemo extends DemoBase
{
    ...
    private static DropSupport createDropSupport( Graph2DView view )
    {
        ...
        DropSupport dropSupport = new DropSupport( view )
        {
            ...
            protected boolean dropNodeRealizer( Graph2DView view, NodeRealizer r, double worldCoordX,
                                                double worldCoordY )
            {
                                                       ...
                                                       new DragGestureListener()
                                                       {
                                                           public void dragGestureRecognized( DragGestureEvent event )
                                                           {
                                                               ...
                                                               else if ( value instanceof NodeRealizer )
                                                               {
                                                                   ...
                                                                   support.startDrag( dragSource, nr, event,
                                                                                      DragSource.DefaultMoveDrop );
                                                               }
                                                           }
                                                       } );
    ...
    protected JToolBar createToolBar()
    {
        ...
        final Icon undoIcon = getIconResource( "resource/undo.png" );
        if ( undoIcon != null )
        {
            undoAction.putValue( Action.SMALL_ICON, undoIcon );
            undoAction.putValue( Action.SHORT_DESCRIPTION, "Undo" );
        }
        toolBar.add( createActionControl( undoAction, false ) );
        final Action redoAction = undoManager.getRedoAction();
        final Icon redoIcon = getIconResource( "resource/redo.png" );
        if ( redoIcon != null )
        {
            redoAction.putValue( Action.SMALL_ICON, redoIcon );
            redoAction.putValue( Action.SHORT_DESCRIPTION, "Redo" );
        }
        toolBar.add( createActionControl( redoAction, false ) );
...
class DropItemListCellRenderer implements ListCellRenderer
{
    ...
    )
    {
        ...
        {
            final Component c =
                    realizerRenderer.getListCellRendererComponent( list, value, index, isSelected, cellHasFocus );
