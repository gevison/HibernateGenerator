...
import y.view.LocalViewCreator.AbstractLocalViewCreator;
import y.view.LocalViewCreator.AncestorGroups;
import y.view.LocalViewCreator.CommonParentGroup;
import y.view.LocalViewCreator.EdgeGroup;
import y.view.LocalViewCreator.FolderContents;
import y.view.LocalViewCreator.Neighborhood;
import y.view.LocalViewCreator.SourceAndTarget;
...
public class LocalViewDemo extends DemoBase
{
                    ...
                    { // we clicked on an edge
                        ...
                        if ( modelEdge != null )
                        {
                            if ( modelEdge.getGraph() == viewGraph )
                            {
                                ...
                                double minZoom =
                                        Math.min(
                                                view.getWidth() / ( snr.getWidth() + tnr.getWidth() +
                                                                            Math.abs( sCenter.x - tCenter.x ) ),
                                                view.getHeight() / ( snr.getHeight() + tnr.getHeight() +
                                                                             Math.abs( sCenter.y - tCenter.y ) ) );
                                view.focusView( Math.min( view.getZoom(), minZoom ),
                                                new Point2D.Double( ( sCenter.x + tCenter.x ) * 0.5d,
                                                                    ( sCenter.y + tCenter.y ) * 0.5d ), true );
                                viewGraph.unselectAll();
    ...
    private JComponent createSettingsComponent()
    {
        ...
        addStrategy( settingsPane, nodeStrategyButtons, nodeStrategiesGroup, "Neighborhood",
                     createNeighborhoodSettings(), neighborhood, true );
        addStrategy( settingsPane, nodeStrategyButtons, nodeStrategiesGroup, "Common Parent Group",
                     createCommonParentGroupSettings(), commonParentGroup, true );
        addStrategy( settingsPane, nodeStrategyButtons, nodeStrategiesGroup, "Ancestor Groups",
                     createAncestorGroupsSettings(), ancestorGroups, true );
        addStrategy( settingsPane, nodeStrategyButtons, nodeStrategiesGroup, "Folder Contents",
                     createFolderContentsSettings(), folderContents, true );
        addStrategy( settingsPane, nodeStrategyButtons, nodeStrategiesGroup, "Selected Subgraph", null,
                     selectedSubgraph, true );
        ...
        addStrategy( settingsPane, edgeStrategyButtons, edgeStrategiesGroup, "Source and Target",
                     createSourceAndTargetSettings(), sourceAndTarget, false );
        addStrategy( settingsPane, edgeStrategyButtons, edgeStrategiesGroup, "Edge Group", createEdgeGroupSettings(),
                     edgeGroup, false );
