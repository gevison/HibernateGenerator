...
{
    private static Color defaultEdgeColor = new Color( 202, 227, 255 );

    private static LineType defaultEdgeLineType = LineType.LINE_3;
    ...
    protected void configureDefaultRealizers()
    {
        ...
        implementationsMap.put( GenericEdgeRealizer.BendPainter.class,
                                new CustomBendPainter( new Ellipse2D.Double( 0, 0, 10, 5 ),
                                                       new Ellipse2D.Double( 0, 0, 10, 10 ), Color.blue,
                                                       Color.red ) );
    ...
    protected JList createEdgeRealizerList()
    {
        ...
        result.setCellRenderer( new RealizerCellRenderer( CELL_WIDTH, CELL_HEIGHT )
        {
            ...
            {
                RealizerCellRenderer.EdgeRealizerIcon icon =
                        new RealizerCellRenderer.EdgeRealizerIcon( realizer, iconWidth,
                                                                   iconHeight )
                        {
                            protected void paintRealizer( EdgeRealizer edgeRealizer, Graphics2D gfx )
                            {
                                ...
                                sourceNodeRealizer.setCenter( 15.0, calculateSourceBend( edgeRealizer, CELL_WIDTH,
                                                                                         CELL_HEIGHT ).getY() );
                                ...
                                targetNodeRealizer.setCenter( CELL_WIDTH - 15.0,
                                                              calculateTargetBend( edgeRealizer, CELL_WIDTH,
                                                                                   CELL_HEIGHT ).getY() );
    ...
    static final class MultiColorEdgePainter extends GenericEdgePainter
    {
        ...
        {
            float[] colorInHsb =
                    Color.RGBtoHSB( currentColor.getRed(), currentColor.getGreen(), currentColor.getBlue(), null );
    ...
    static final class MyPathCalculator extends PolyLinePathCalculator implements GenericEdgeRealizer.PathCalculator
    {
                                   ...
                                   Point2D targetIntersectionPointOut )
        {
            ...
            {
                return super
                        .calculatePath( context, bends, path, sourceIntersectionPointOut, targetIntersectionPointOut );
            }
            else
            {
     ...
     */
    static final class UndulatingPathCalculator extends PolyLinePathCalculator implements
                ...
                if ( len > 0 )
                {
                    ...
                    for ( int i = 0; i < count; i++ )
                    {
                        ...
                        scratch.quadTo( lastPointX + ( i + 0.5f ) / ( ( float ) count ) * dx + dy * height / len,
                                        ...
                                        lastPointX + ( i + 1 ) / ( ( float ) count ) * dx,
                                        lastPointY + ( i + 1 ) / ( ( float ) count ) * dy );
                    }
                }
                else
                {
    ...
    static final class CustomBendPainter implements GenericEdgeRealizer.BendPainter
    {
        ...
        public void paintBends( EdgeRealizer context, BendList bends, GeneralPath path, Graphics2D gfx,
                                boolean selected )
        {
            if ( !bends.isEmpty() )
            {
                ...
                for ( BendCursor bendCursor = bends.bends(); bendCursor.ok(); bendCursor.next() )
                {
                    ...
                    gfx.setColor( ( ( selected || b.isSelected() ) && useSelectionStyle ) ? this.selectedFillColor
                                                                                          : this.fillColor );
                    final double x = b.getX();
                    final double y = b.getY();
                    RectangularShape shape = selected ? this.selectedShape : this.shape;
                    shape.setFrame( x - shape.getWidth() / 2, y - shape.getHeight() / 2, shape.getWidth(),
                                    shape.getHeight() );
                    gfx.fill( shape );
                }
                gfx.setColor( oldColor );
