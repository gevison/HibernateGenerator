...
public class EdgeConnectorDemo extends DemoBase
{
    ...
    protected GraphMLIOHandler createGraphMLIOHandler()
    {
        ...
        ioHandler.getGraphMLHandler().addOutputHandlerProvider(
                new AbstractOutputHandler( "edgeConnectingData", KeyScope.NODE, KeyType.COMPLEX )
                {
                            ...
                            throws GraphMLWriteException
                    {
                        if ( data != null )
                        {
                            ...
                            GraphElementIdProvider idProvider =
                                    ( GraphElementIdProvider ) context.lookup( GraphElementIdProvider.class );
        ...
        ioHandler.getGraphMLHandler()
                 .addOutputHandlerProvider( new AbstractOutputHandler( "edgeId", KeyScope.EDGE, KeyType.STRING )
                 {
                     protected void writeValueCore( GraphMLWriteContext context, Object data )
                             throws GraphMLWriteException
                     {
        ...
        ioHandler.getGraphMLHandler().addInputDataAcceptor( "edgeConnectingData", tempConnectorMap, KeyScope.NODE,
                                                            new NameBasedDeserializer()
                                                            {
        ...
        public void mouseShiftPressedLeft( double x, double y )
        {
            ...
            else
            {
                ...
                if ( edge != null )
                {
                    ...
                    double[] result =
                            PointPathProjector.calculateClosestPathPoint( graph.getRealizer( edge ).getPath(), p );
    ...
    static class EdgeConnectorMovePortMode extends MovePortMode
    {
        ...
        protected YList getPortCandidates( Node v, Edge e, double gridSpacing )
        {
            ...
            if ( connectedEdge != null )
            {
                ...
                double[] pppResult = PointPathProjector
                        .calculateClosestPathPoint( getGraph2D().getRealizer( connectedEdge ).getPath(), p );
        ...
        public void mouseReleasedLeft( double x, double y )
        {
            ...
            if ( p != null )
            {
                ...
                {
                    double[] result = PointPathProjector
                            .calculateClosestPathPoint( getGraph2D().getRealizer( connectedEdge ).getPath(), x, y );
