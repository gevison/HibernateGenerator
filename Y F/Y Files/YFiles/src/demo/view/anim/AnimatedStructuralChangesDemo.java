            ...
            if ( newCount > 1 && oldCount > 1 )
            {
                ...
                for ( int i = 0, n = random.nextInt( MAX_EDGE_COUNT + 1 - graph.edgeCount() ); i < n; ++i )
                {
                    ...
                    else if ( d < 0.5 )
                    {
                        ...
                        edge = graph.createEdge( nodes[ random.nextInt( oldCount ) ],
                                                 nodes[ oldCount + random.nextInt( newCount ) ] );
                    }
                    else if ( d < 0.9 )
                    {
                        // create an edge between a new and an old node
                        edge = graph.createEdge( nodes[ oldCount + random.nextInt( newCount ) ],
                                                 nodes[ random.nextInt( oldCount ) ] );
                    }
                    else
                    {
