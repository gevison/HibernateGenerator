...
class LayoutSupport
{
    ...
    static void doGroupedOrthogonalLayout( final LayoutContext context )
    {
        ...
        if ( context.isFromSketch() )
        {
            ...
            ( ( OrthogonalEdgeRouter ) pl.getEdgeRouter() ).setCrossingCost( 0 );
            ...
            Layouter layouter = new Layouter()
            {
                ...
                public void doLayout( LayoutGraph graph )
                {
                    ...
                    graph.addDataProvider( PartialLayouter.PARTIAL_EDGES_DPKEY,
                                           DataProviders.createConstantDataProvider( Boolean.TRUE ) );
                    graph.addDataProvider( PartialLayouter.PARTIAL_NODES_DPKEY,
    ...
    static void doOrthogonalLayout( final LayoutContext context )
    {
        ...
        if ( context.isFromSketch() )
        {
            ...
            pl.setConsiderNodeAlignment( false );
            pl.setMinimalNodeDistance( 20 );
            ( ( OrthogonalEdgeRouter ) pl.getEdgeRouter() ).setCrossingCost( 2 );
