...
//    YLabel.setFractionMetricsForSizeCalculationEnabled(true);
    ...
    protected void addMouseInteraction()
    {
        ...
        {
            /**
             * Overwrites {@link TooltipMode#getNodeTip(y.base.Node)} to set a tooltip text with some information about an
             * employee if the level of detail is set to {@link LODRenderingHint#VALUE_LOD_LOW} or {@link
             * LODRenderingHint#VALUE_LOD_MEDIUM}.
             *
             * @param node the node for which the tooltip is set.
             * @return a tooltip text if the level of detail is low or medium, <code>null</code> otherwise.
             */
            protected String getNodeTip( Node node )
            {
                String tipText = null;
                Object lod = view.getRenderingHints().get( LODRenderingHint.KEY_LOD );
                if ( lod == LODRenderingHint.VALUE_LOD_LOW || lod == LODRenderingHint.VALUE_LOD_MEDIUM )
                { //tiny
                    if ( getGraph2D().getHierarchyManager().isNormalNode( node ) )
                    {
                        Employee e = ( Employee ) ( ( JOrgChart ) view ).getUserObject( node );
                        tipText = "<html><b>" + e.name + "</b><br>" + e.position + "<br>Status " + e.status + "</html>";
                    }
                }
                return tipText;
            }
            ...
            /**
             * Overwrites {@link TooltipMode#getNodeLabelTip(y.view.NodeLabel)} to set a tooltip text with the employee's
             * presence status if the level of detail is set to {@link LODRenderingHint#VALUE_LOD_HIGH} and the given label
             * equals the status label in the upper right corner of the node.
             * @param label the label for which the tooltip is set.
             * @return a tooltip text for the status label if the level of detail is high, <code>null</code> otherwise.
             */
            protected String getNodeLabelTip( NodeLabel label )
            {
                String tipText = null;
                Object lod = view.getRenderingHints().get( LODRenderingHint.KEY_LOD );
                if ( lod == LODRenderingHint.VALUE_LOD_HIGH )
                {
                    NodeRealizer realizer = label.getRealizer();
    ...
    protected void setRealizerDefaults()
    {
        ...
        DefaultHierarchyGraphFactory gf =
                ( DefaultHierarchyGraphFactory ) getGraph2D().getHierarchyManager().getGraphFactory();
    ...
    protected void configureNodeRealizer( Node n )
    {
        ...
        { //NOT CONFIGURED YET
            ...
            label.setModelParameter( model.createDiscreteModelParameter( SmartNodeLabelModel.POSITION_TOP_LEFT ) );
            label.setInsets( new Insets( 2, 2, 2, 2 ) );

            //LABEL 1-5: EMPLOYEE INFORMATION
            label = addTextLabel( gnr, employee.name, 65, 4, true );
            label = addTextLabel( gnr, employee.position, 65, label.getOffsetY() + label.getHeight() + 4,
                                  true ); // label 2
            label = addTextLabel( gnr, employee.email, 65, label.getOffsetY() + label.getHeight() + 4,
                                  true ); // label 3
            label = addTextLabel( gnr, employee.phone, 65, label.getOffsetY() + label.getHeight() + 4,
                                  true ); // label 4
            ...
            label.setLabelModel( model );
            label.setModelParameter( model.createDiscreteModelParameter( SmartNodeLabelModel.POSITION_TOP_RIGHT ) );
            label.setInsets( new Insets( 2, 2, 2, 2 ) );
            ...
            model = new SmartNodeLabelModel();
            label.setLabelModel( model );
            label.setModelParameter( model.createDiscreteModelParameter( SmartNodeLabelModel.POSITION_CENTER ) );
    ...
    NodeLabel addTextLabel( NodeRealizer nr, String text, double x, double y, boolean cropping )
    {
        ...
        OrientedRectangle box = new OrientedRectangle( nr.getX() + x, nr.getY() + y + h, label.getWidth(), h );
        label.setModelParameter(
                labelModel.createAlignedModelParameter( box, nr, SmartNodeLabelModel.ALIGNMENT_TOP_LEFT ) );
    ...
    static class StateIcon implements Icon
    {
        ...
        public void paintIcon( Component c, Graphics g, int x, int y )
        {
            ...
            else if ( lod == LODRenderingHint.VALUE_LOD_HIGH )
            {
                ...
                circle.setFrame( circle.x + delta, circle.y + delta, circle.width - 2 * delta,
                                 circle.height - 2 * delta );
    ...
    static class EmployeePainter extends ShapeNodePainter
    {
        ...
        public void paint( NodeRealizer context, Graphics2D gfx )
        {
            ...
            if ( lod == LODRenderingHint.VALUE_LOD_OVERVIEW )
            {
                ...
                Rectangle2D rect =
                        new Rectangle2D.Double( context.getX() + 20, context.getY() + 20, context.getWidth() - 40,
                                                context.getHeight() - 40 );
                gfx.setColor( ovc );
                gfx.fill( rect );
            ...
            { //defaults to LODRenderingHint.VALUE_LOD_HIGH)
        ...
        protected void paintFilledShape( NodeRealizer context, Graphics2D graphics, Shape shape )
        {
            ...
            boolean selected = useSelectionStyle( context, graphics );
            Color c1 = getFillColor( context, selected );

            if ( c1 != null && !context.isTransparent() )
            {
                Color c2 = getFillColor2( context, selected );
    ...
    private class RollOverViewMode extends ViewMode
    {
        ...
        class CanvasPropertyChangeListener implements PropertyChangeListener
        {
            public void propertyChange( PropertyChangeEvent evt )
            {
                if ( lastRelevantMouseEvent != null &&
                        ( "Zoom".equals( evt.getPropertyName() ) || "ViewPoint".equals( evt.getPropertyName() ) ) )
                {
        ...
        public void onGraph2DSelectionEvent( Graph2DSelectionEvent ev )
        {
            if ( ev.getSubject() instanceof Node )
            {
                ...
                if ( userObject != null )
                {
                    ...
                    configureColors( getGraph2D().getRealizer( node ), graph.isSelected( node ),
                                     highlightMap.getBool( node ) );
                    graph.updateViews();
                }
            }
        }
