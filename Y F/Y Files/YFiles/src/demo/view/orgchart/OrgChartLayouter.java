...
public class OrgChartLayouter implements Layouter
{
    ...
    protected NodePlacer createNodePlacer( Object childLayout )
    {
        ...
        {
            DefaultNodePlacer placer = new DefaultNodePlacer( DefaultNodePlacer.PLACEMENT_HORIZONTAL_DOWNWARD,
                                                              DefaultNodePlacer.ALIGNMENT_CENTER,
                                                              DefaultNodePlacer.ROUTING_FORK, 20.0d, 80.d );
            ...
            for ( NodeCursor nc = graph.nodes(); nc.ok(); nc.next() )
            {
                ...
                for ( EdgeCursor ec = n.outEdges(); ec.ok(); ec.next() )
                {
                    ...
                    if ( er.pointCount() > 0 )
                    {
                        ...
                        loop:
                        for ( EdgeCursor ecc = n.outEdges(); ecc.ok(); ecc.next() )
                        {
