...
class FlowchartAlignmentCalculator
{
    ...
    public void determineAlignment( LayoutGraph graph, LayoutDataProvider ldp, NodeMap nodeAlignment,
                                    EdgeMap edgeLength )
    {
...
//    }
    ...
    private boolean isAlignable( GraphInterface graph, LayoutDataProvider ldp, Edge edge )
    {
        ...
        if ( hasFlatwisePortConstraint( edgeData ) ||
                hasFlatwiseCandidateCollection( edgeData, getLayoutOrientation() ) )
        {
            return false;
        }
                                       ...
                                       DataProvider edgeIsAlignable, EdgeMap edgeLength )
    {
        ...
        for ( NodeCursor nc = graph.nodes(); nc.ok(); nc.next() )
        {
            ...
            if ( node.outDegree() == 2 && node.inDegree() == 2 )
            {
                ...
                final boolean preventFirstIn =
                        !edgeIsAlignable.getBool( firstIn ) || !edgeIsAlignable.getBool( lastOut );
                final boolean preventFirstOut =
                        !edgeIsAlignable.getBool( firstOut ) || !edgeIsAlignable.getBool( lastIn );
...
//                System.out.println("* " + ec.edge() + " l:" + edgeLength.getDouble(ec.edge()));
    ...
    private static boolean hasFlatwiseCandidateCollection( final EdgeData edgeData, final byte layoutOrientation )
    {
        return FlowchartPortOptimizer.isFlatwiseCandidateCollection( edgeData.getSourceCandidates(), layoutOrientation )
                || FlowchartPortOptimizer
                .isFlatwiseCandidateCollection( edgeData.getTargetCandidates(), layoutOrientation );
    }
    ...
    static class NodeAlignmentCalculator
    {
                                             ...
                                             DataProvider edgePriority, DataAcceptor node2AlignWith )
        {
            ...
            for ( EdgeCursor ec = FlowchartPortOptimizer.getSameLayerEdges( graph, ldp ).edges(); ec.ok(); ec.next() )
            {
                        ...
                        ( !isGroupNodeBorder( e.source(), ldp ) || !isGroupNodeBorder( e.target(), ldp ) ) )
                {
                    ...
                    Node tRep = ( Node ) node2NetworkRep.get( e.target() );
                    Edge connector =
                            ldp.getNodeData( e.source() ).getPosition() < ldp.getNodeData( e.target() ).getPosition() ?
            ...
            for ( int i = 0; i < nodes.length; i++ )
            {
                ...
                if ( last != null && areInSameLayer( last, n, ldp ) )
                {
                    ...
                    {
                        Edge connector =
                                network.createEdge( lastRep, nRep ); //guarantees that last is placed to the left of n
            ...
            for ( NodeCursor nc = graph.nodes(); nc.ok(); nc.next() )
            {
                ...
                if ( !network.containsEdge( nRep, globalSink ) )
                {
                    ...
                    edgeWeight.setInt( globalSinkConnector,
                ...
                if ( !network.containsEdge( globalSource, nRep ) )
                {
                    ...
                    edgeWeight.setInt( globalSourceConnector,
        ...
        private static int calcMinLength( Node n1, Node n2, LayoutGraph graph, LayoutDataProvider ldp )
        {
            ...
            if ( isGroupNodeBorder( n1, ldp ) && isGroupNodeBorder( n2, ldp ) )
            {
                ...
                if ( n1GroupNode != n2GroupNode && node2Parent.get( n1 ) != n2GroupNode &&
                        node2Parent.get( n2 ) != n1GroupNode )
                {
                    return 1;
                }
                else
            ...
            {
                Node n1GroupNode = ldp.getNodeData( n1 ).getGroupNode();
                Node n2GroupNode = isGroupNodeProxy( n2, ldp ) ? ldp.getNodeData( n2 ).getGroupNode()
                                                               : ( Node ) node2Parent.get( n2 );
            ...
            {
                Node n1GroupNode = isGroupNodeProxy( n1, ldp ) ? ldp.getNodeData( n1 ).getGroupNode()
                                                               : ( Node ) node2Parent.get( n1 );
                Node n2GroupNode = ldp.getNodeData( n2 ).getGroupNode();
                                                  ...
                                                  LayoutDataProvider ldp )
        {
            ...
            for ( NodeCursor nc = seenDummyNodes.nodes(); nc.ok(); nc.next() )
            {
                ...
                {
                    node2AlignmentLayer
                            .setDouble( nc.node(), dummyAlignmentLayer - 0.5 ); //assign dummy nodes to a separate layer
                }
            }
        }
        ...
        private static boolean isGroupNodeBorder( Node n, LayoutDataProvider ldp )
        {
            final NodeData data = ldp.getNodeData( n );
            return data != null &&
                    ( data.getType() == NodeData.TYPE_GROUP_BEGIN || data.getType() == NodeData.TYPE_GROUP_END );
        }
        ...
        static class AlignedNodePositionComparator implements Comparator
        {
            ...
            public int compare( Object o1, Object o2 )
            {
                ...
                {
                    return Comparators.compare( ldp.getNodeData( ( Node ) o1 ).getLayer(),
                                                ldp.getNodeData( ( Node ) o2 ).getLayer() );
                }
            }
        }
