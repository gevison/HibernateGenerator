...
class FlowchartTransformerStage extends AbstractLayoutStage
{
    ...
    public void doLayout( LayoutGraph graph )
    {
        ...
        final DataProvider backupSourceConstraintsDP =
                graph.getDataProvider( PortConstraintKeys.SOURCE_PORT_CONSTRAINT_KEY );
        final DataProvider backupTargetConstraintsDP =
                graph.getDataProvider( PortConstraintKeys.TARGET_PORT_CONSTRAINT_KEY );
        ...
        graph.addDataProvider( Layouter.NODE_ID_DPKEY, backupNodeIdDP != null ?
                                                       ( DataProvider ) new WrappedObjectDataProvider( backupNodeIdDP,
                                                                                                       provider )
                                                                              : provider );
        ...
        finally
        {
            ...
            {
                FlowchartLayouter
                        .restoreDataProvider( graph, groupNodeIdWrapper.getFallback(), GroupingKeys.NODE_ID_DPKEY );
            }
    ...
    void configurePreferredEdgeDirections( final LayoutGraph graph )
    {
        ...
        for ( NodeCursor nodeCursor = graph.nodes(); nodeCursor.ok(); nodeCursor.next() )
        {
            ...
            for ( Edge edge = node.firstOutEdge(); edge != null; edge = edge.nextOutEdge() )
            {
                final int dir = directions.getInt( edge );
                if ( dir == FlowchartLayouter.DIRECTION_LEFT_IN_FLOW ||
                        dir == FlowchartLayouter.DIRECTION_RIGHT_IN_FLOW )
                {
    ...
    class InEdgeGroupingConfigurator
    {
        ...
        EdgeList createBus( LayoutGraph graph, EdgeList[] layers )
        {
            ...
            for ( int i = 0; i < layers.length; i++ )
            {
                ...
                if ( nonSingletonLayerEdges.isEmpty() && layer.size() == 1 )
                {
                    ...
                    {
                        final Node layerDummy =
                                createDummyNode( graph, getGroupingType(), getLayerId( edge.source() ) );
        ...
        {
            targetCandidates
                    .set( edge, createStrongPortCandidate( edge, false, PortCandidate.AGAINST_THE_FLOW, graph ) );
        }
    ...
    class SucceedingLayersInEdgeGroupingConfigurator extends InEdgeGroupingConfigurator
    {
        ...
        {
            targetCandidates
                    .set( edge, createStrongPortCandidate( edge, true, PortCandidate.AGAINST_THE_FLOW, graph ) );
        }
        ...
        void createGrouping( EdgeList nonBusEdges, Node neighborLayerNode, LayoutGraph graph )
        {
            ...
            for ( EdgeCursor ec = nonBusEdges.edges(); ec.ok(); ec.next() )
            {
                ...
                sourceCandidates.set( groupingEdge,
                                      createStrongPortCandidate( groupingEdge, false, PortCandidate.WITH_THE_FLOW,
                                                                 graph ) );
            }
        }
        ...
        void prepareForGrouping( EdgeList nonBusEdges, LayoutGraph graph )
        {
            ...
            sourceCandidates.set( sameLayerEdge,
                                  createStrongPortCandidate( sameLayerEdge, true, PortCandidate.AGAINST_THE_FLOW,
                                                             graph ) );
