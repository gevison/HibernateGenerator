...
public class AnimatedNavigationDemo extends NavigationDemo
{
    ...
    protected void moveToCenter( final Node newCenterNode, boolean animated )
    {
        ...
        for ( EdgeCursor ec = graph.edges(); ec.ok(); ec.next() )
        {
            ...
            final boolean shouldBeHidden =
                    hiddenNodesMap.getBool( edge.source() ) || hiddenNodesMap.getBool( edge.target() );
            if ( shouldBeHidden )
            {
        ...
        { //Fallback: Use the informations from the realizers (e.g. at the beginning)
    ...
    private void animateCamera( double x, double y )
    {
        ...
        AnimationObject animationObject =
                factory.moveCamera( DefaultMutableValue2D.create( x, y ), PREFERRED_DURATION );
