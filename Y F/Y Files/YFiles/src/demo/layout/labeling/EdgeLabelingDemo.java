...
public class EdgeLabelingDemo extends DemoBase
{
    ...
    void updateEdgeLabels()
    {
        ...
        final SmartEdgeLabelModel edgeLabelModel =
                new SmartEdgeLabelModel(); //the model that specifies the dynamic behavior of the label
        edgeLabelModel.setAutoRotationEnabled( optionHandler.getBool( AUTO_ROTATE_STRING ) );
        final CompositeEdgeLabelModel labelingModel =
                getCurrentEdgeLabelModel(); //the model used by the labeling algorithm (can be different from the edge label model)
        ...
        for ( EdgeCursor ec = graph.edges(); ec.ok(); ec.next() )
        {
            ...
            for ( int i = 0; i < labelLayouts.length; i++ )
            {
                ...
                final Object parameter = edgeLabelModel.createModelParameter( label.getOrientedBox(),
                                                                              graph.getEdgeLayout( edge ),
                                                                              graph.getNodeLayout( edge.source() ),
                                                                              graph.getNodeLayout(
                                                                                      edge.target() ) ); //identify the parameter that fits the current position best
    ...
    private CompositeEdgeLabelModel getCurrentEdgeLabelModel()
    {
        ...
        compositeEdgeLabelModel.add( getEdgeLabelModel( modelId, optionHandler.getBool( AUTO_ROTATE_STRING ),
                                                        optionHandler.getDouble( EDGE_TO_LABEL_DISTANCE_STRING ),
                                                        angle ) );
        ...
        public JPopupMenu getEdgePopup( final Edge edge )
        {
            ...
            pm.add( new AbstractAction( "Add Label" )
            {
                ...
                public void actionPerformed( ActionEvent e )
                {
                    ...
                    label2Model
                            .put( label, getCurrentEdgeLabelModel() ); //sets the model used by the labeling algorithm
                    label.setConfiguration( CUSTOM_LABELING_CONFIG_NAME );
