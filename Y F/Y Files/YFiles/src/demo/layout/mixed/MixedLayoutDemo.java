...
public class MixedLayoutDemo extends GroupingDemo
{
    ...
    void applyTableLayout()
    {
        ...
        graph.addDataProvider( PortCandidate.SOURCE_PCLIST_DPKEY,
                               DataProviders.createConstantDataProvider( candidates ) );
        graph.addDataProvider( PortCandidate.TARGET_PCLIST_DPKEY,
                               DataProviders.createConstantDataProvider( candidates ) );

        //configure layout algorithms
        final RowLayouter rowLayouter =
                new RowLayouter(); //used for layouting the nodes (rows) within the group nodes (tables)
    ...
    void applyThreeTierLayout()
    {
        ...
        final IncrementalHierarchicLayouter partitionLayouter =
                new IncrementalHierarchicLayouter(); //configure the core layout
