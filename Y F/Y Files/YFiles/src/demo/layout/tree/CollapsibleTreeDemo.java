...
public class CollapsibleTreeDemo extends DemoBase
{
    ...
    public CollapsibleTreeDemo()
    {
        ...
        treeLayouter
                .setComparator( new XCoordComparator() ); //important to keep node order of collapsed/expanded items.
    ...
    protected void configureDefaultRealizers()
    {
        ...
        nl.setModelParameter( model.createDiscreteModelParameter( SmartNodeLabelModel.POSITION_CENTER ) );
    ...
    class CollapseExpandViewMode extends ViewMode
    {
        ...
        public void mouseClicked( MouseEvent ev )
        {
            ...
            if ( node != null )
            {
                ...
                {
                    if ( ev.isControlDown() )
                    {//ctrl is pressed expand whole subtree (max depth of 10000) of current node
                        expandSubtree( getGraph2D(), node, 10000 );
                    }
                    else
                    {//ctrl is not pressed expand only current node
                ...
                {
                    if ( ev.isControlDown() )
                    {//ctrl is pressed collapse whole subtree of current node
                        collapseSubtree( getGraph2D(), node );
                    }
                    else
                    {//ctrl is not pressed collapse only current node
        ...
        {
            EdgeList edgeList =
                    collapsedEdges.get( root ) != null ? ( EdgeList ) collapsedEdges.get( root ) : new EdgeList();
    ...
    void layout( Graph2D graph2D, final Node focusNode, boolean fitContent )
    {
        ...
        graph2D.addDataProvider( FocusNodeLayoutStage.FOCUS_NODE_DPKEY,
                                 FocusNodeLayoutStage.createFocusNodeDataProvider( focusNode ) );
