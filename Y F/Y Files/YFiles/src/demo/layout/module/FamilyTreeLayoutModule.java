...
//  private static final String BREAK_LONG_SEGMENTS = "BREAK_LONG_SEGMENTS";
    ...
    private static final Object[] sortBySexEnum = { DO_NOT_SORT, FEMALE_LEFT, FEMALE_ALWAYS_LEFT, MALE_LEFT,
                                                    MALE_ALWAYS_LEFT };
...
//    op.addBool(BREAK_LONG_SEGMENTS, true);
    ...
    /**
     * Configures an instance of FamilyTreeLayouter. The values provided by this module's option handler are being used
     * for this purpose.
     */
    public void configure( Layouter layouter )
    {
        ...
        {
            final FamilyTreeLayouter ftl = ( FamilyTreeLayouter ) layouter;
            final OptionHandler op = getOptionHandler();
            final Graph2D graph = getGraph2D();

            ftl.setSpacingBetweenFamilyMembers( op.getDouble( HORIZONTAL_SPACING ) );
            ftl.setOffsetForFamilyNodes( op.getDouble( VERTICAL_SPACING ) );
            ftl.setFamilyNodesAlwaysBelow( op.getBool( FAMILIES_ALWAYS_BELOW ) );
            ftl.setPartnerlessBelow( op.getBool( SINGLE_DIRECT_BELOW ) );
...
//        OrientationLayouter.MIRROR_BOTTOM_TO_TOP | OrientationLayouter.MIRROR_LEFT_TO_RIGHT);
            ...
            {
                ftl.setLayoutOrientation( LayoutOrientation.TOP_TO_BOTTOM );
            }
            else if ( op.get( ORIENTATION ).equals( LEFT_TO_RIGHT ) )
            {
                ftl.setLayoutOrientation( LayoutOrientation.LEFT_TO_RIGHT );
            }
            else if ( op.get( ORIENTATION ).equals( BOTTOM_TO_TOP ) )
            {
                ftl.setLayoutOrientation( LayoutOrientation.BOTTOM_TO_TOP );
            }
            else if ( op.get( ORIENTATION ).equals( RIGHT_TO_LEFT ) )
            {
                ftl.setLayoutOrientation( LayoutOrientation.RIGHT_TO_LEFT );
            }

      /* Advanced */

      /* Component Layouter */
            ftl.setComponentLayouterEnabled( op.getBool( USE_COMPONENT_LAYOUTER ) );
            ComponentLayouter cl = ( ComponentLayouter ) ftl.getComponentLayouter();
      ...
      /* Vertical node alignment */
            ...
            {
                ftl.setSortFamilyMembers( FamilyTreeLayouter.DO_NOT_SORT_BY_SEX );
            }
            else if ( op.get( SORT_BY_SEX ).equals( FEMALE_LEFT ) )
            {
                ftl.setSortFamilyMembers( FamilyTreeLayouter.FEMALE_FIRST );
            }
            else if ( op.get( SORT_BY_SEX ).equals( FEMALE_ALWAYS_LEFT ) )
            {
                ftl.setSortFamilyMembers( FamilyTreeLayouter.FEMALE_ALWAYS_FIRST );
            }
            else if ( op.get( SORT_BY_SEX ).equals( MALE_LEFT ) )
            {
                ftl.setSortFamilyMembers( FamilyTreeLayouter.MALE_FIRST );
            }
            else if ( op.get( SORT_BY_SEX ).equals( MALE_ALWAYS_LEFT ) )
            {
                ftl.setSortFamilyMembers( FamilyTreeLayouter.MALE_ALWAYS_FIRST );
            }
