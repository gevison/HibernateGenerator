...
public class CircularLayoutModule extends LayoutModule
{
    ...
    private final static String[] partitionLayoutStyles =
            { PARTITION_LAYOUTSTYLE_CYCLIC, PARTITION_LAYOUTSTYLE_DISK, PARTITION_LAYOUTSTYLE_ORGANIC };
    ...
    public OptionHandler createOptionHandler()
    {
        ...
        cm.setEnabledOnValueEquals( op.getItem( GENERAL, LAYOUT_STYLE ), SINGLE_CYCLE,
                                    op.getItem( TREE, MINIMAL_TREE_NODE_DISTANCE ), true );
        cm.setEnabledOnValueEquals( op.getItem( GENERAL, LAYOUT_STYLE ), SINGLE_CYCLE,
                                    op.getItem( TREE, PLACE_CHILDREN_ON_COMMON_RADIUS ), true );
        cm.setEnabledOnValueEquals( op.getItem( GENERAL, LAYOUT_STYLE ), SINGLE_CYCLE,
                                    op.getItem( TREE, PREFERRED_CHILD_WEDGE ), true );
        cm.setEnabledOnValueEquals( op.getItem( GENERAL, LAYOUT_STYLE ), SINGLE_CYCLE,
                                    op.getItem( TREE, MINIMAL_EDGE_LENGTH ), true );
        cm.setEnabledOnValueEquals( op.getItem( GENERAL, LAYOUT_STYLE ), SINGLE_CYCLE,
                                    op.getItem( TREE, MAXIMAL_DEVIATION_ANGLE ), true );
        cm.setEnabledOnValueEquals( op.getItem( GENERAL, LAYOUT_STYLE ), SINGLE_CYCLE,
                                    op.getItem( TREE, COMPACTNESS_FACTOR ), true );
        cm.setEnabledOnValueEquals( op.getItem( GENERAL, LAYOUT_STYLE ), SINGLE_CYCLE,
                                    op.getItem( TREE, ALLOW_OVERLAPS ), true );
        return op;
    }
