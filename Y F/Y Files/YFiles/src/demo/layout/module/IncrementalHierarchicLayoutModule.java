...
public class IncrementalHierarchicLayoutModule extends LayoutModule
{
    ...
    private static final String[] rankingPolicies =
            { HIERARCHICAL_OPTIMAL, HIERARCHICAL_TIGHT_TREE_HEURISTIC, BFS_LAYERS,
              FROM_SKETCH, HIERARCHICAL_TOPMOST };

    private static final String[] edgeLabeling =
            { EDGE_LABELING_NONE, EDGE_LABELING_GENERIC, EDGE_LABELING_HIERARCHIC };
    ...
    private static final Object[] groupHorizCompactionEnum =
            { GROUP_HORIZONTAL_COMPACTION_NONE, GROUP_HORIZONTAL_COMPACTION_MAX };
    ...
    public void mainrun()
    {
        ...
        boolean selectedElements =
                !Selections.isEdgeSelectionEmpty( graph ) || !Selections.isNodeSelectionEmpty( graph );
        ...
        {
            ( ( SimplexNodePlacer ) ihl.getNodePlacer() )
                    .setGroupCompactionStrategy( SimplexNodePlacer.GROUP_COMPACTION_NONE );
        }
        else if ( GROUP_HORIZONTAL_COMPACTION_MAX.equals( op.getString( GROUP_HORIZONTAL_COMPACTION ) ) )
        {
            ( ( SimplexNodePlacer ) ihl.getNodePlacer() )
                    .setGroupCompactionStrategy( SimplexNodePlacer.GROUP_COMPACTION_MAX );
        }
        ...
        {
            ihl.setFromScratchLayeringStrategy(
