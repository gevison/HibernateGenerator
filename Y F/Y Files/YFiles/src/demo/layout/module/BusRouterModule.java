...
public class BusRouterModule extends LayoutModule
{
     ...
     */
        ...
        for ( EdgeCursor ec = SUBSET.equals( oh.get( SCOPE ) ) ? graph.selectedEdges() : graph.edges(); ec.ok();
              ec.next() )
        {
            final Edge edge = ec.edge();
            // Except for scope PARTIAL, all edges in scope are movable
            final boolean fixed =
                    scopePartial && !graph.isSelected( edge.source() ) && !graph.isSelected( edge.target() );
