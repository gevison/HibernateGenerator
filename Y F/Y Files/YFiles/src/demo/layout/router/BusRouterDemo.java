...
public class BusRouterDemo extends DemoBase
{
    ...
    public BusRouterDemo( final String helpFilePath )
    {
        ...
        demoTools.setViewAndRouter( view, hubRoutingSupport.getModule().getBusRouter() );
        demoTools.updateGrid();
        demoTools.updateSnapping();

        JSplitPane mainSplitPane =
                new JSplitPane( JSplitPane.HORIZONTAL_SPLIT, demoTools.createOptionComponent(), view );
    ...
    protected class AutoRoutingCreateEdgeMode extends HubCreateEdgeMode
    {
        ...
        protected Edge splitSingleBusEdge( Edge edge )
        {
            ...
            try
            {
                ...
                graph.setCenter( hub,
                                 YPoint.midPoint( graph.getSourcePointAbs( edge ), graph.getTargetPointAbs( edge ) ) );
